(() => {
  'use strict';
  const VERSION = '11.0.0-boss-ticket-shop-only';
  ...
  const defaultState = () => ({
    ...
    boss:{active:false,name:'',hp:0,max:0,ends:0,type:'normal',damage:0}, tickets:{boss:0},
  ...
  function startBoss(type='normal'){
    ...
    const isAutoEventBoss=type==='welison';
    if(!isAutoEventBoss){
    ...
    state.boss={active:true,type,name:eventWelison?'💙 Boss Azul Welison':weekly?'💀 Boss Semanal Supremo':'💀 Rei Caveira do Pomar',hp:max,max,ends:now()+HOUR,damage:0};
    applyVisualState(); showBossCinematic(state.boss.name); toast(`${state.boss.name} apareceu por 1 hora${isAutoEventBoss?'':' • -1 ticket'}.`); spawnBossPulse(); sound(120,.28,'sawtooth'); dirty=true; render();
  }
  ...
  function killBoss(){ const reward=Math.floor((state.boss.max/9) * (state.boss.type==='weekly'?2.2:1) * (state.event.welisonEnds>now()?2:1)); state.fruits+=reward; state.stats.total+=reward; state.stats.bossKills++; ensureMissions(); state.missions.daily.boss=(state.missions.daily.boss||0)+1; addPassXp(120); toast(`Boss derrotado! +${fmt(reward)} maçãs. Ticket não volta.`); state.boss.active=false; state.boss.hp=0; sound(820,.25); applyVisualState(); dirty=true; render(); }
  ...
  function redeemCode(){ const code=(dom.codeInput.value||'').trim().toUpperCase(); const rewards={V10CINEMA:{fruits:50000,xp:100},WELISON5X:{fruits:25000,xp:80},MACADOURADA:{skin:'gold',fruits:10000},BOSSUPDATE:{fruits:45000,xp:120}}; const r=rewards[code]; if(!r){ dom.codeShowcase?.classList.add('code-error'); setTimeout(()=>dom.codeShowcase?.classList.remove('code-error'),520); return toast('Código inválido.'); } if(state.codes[code]) return toast('Código já usado.'); state.codes[code]=true; if(r.fruits){state.fruits+=r.fruits;state.stats.total+=r.fruits;} if(r.xp) addPassXp(r.xp); if(r.skin){state.unlockedSkins[r.skin]=true;state.skin=r.skin;} toast(`Código ${code} resgatado!`); dom.codeShowcase?.classList.remove('code-success'); void dom.codeShowcase?.offsetWidth; dom.codeShowcase?.classList.add('code-success'); sound(840,.18); setTimeout(()=>sound(1040,.12,'triangle'),120); dirty=true; dom.codeInput.value=''; if(dom.featuredCodeChip && code===featuredCode){ dom.featuredCodeChip.classList.add('used'); } render(); }
  ...
  function renderShop(){ dom.shopSubtitle.textContent=world().shop; dom.shopList.innerHTML=upgrades.map(u=>{ const lvl=u.special?state.prestige:(state.up[u.id]||0), cost=upgradeCost(u); return `<div class="item"><div class="icon">${u.icon}</div><div><h3>${u.name} <small>Lv ${lvl}${u.cap?'/'+u.cap:''}</small></h3><p>${u.desc}</p></div><div><div class="price">${fmt(cost)} 🍎</div><button class="primary" data-buy="${u.id}">${u.special?'Prestigiar':'Comprar'}</button></div></div>`; }).join('') + `<div class="item"><div class="icon">🎟️</div><div><h3>Ticket Boss</h3><p>Compra +1 ticket exclusivo para invocar boss. Boss não devolve ticket.</p></div><div><div class="price">${fmt(60000*Math.max(1,state.tickets.boss+1))} 🍎</div><button class="primary" id="buyTicket">Comprar</button></div></div>`; $$('[data-buy]').forEach(b=>b.onclick=()=>buyUpgrade(b.dataset.buy)); $('#buyTicket').onclick=buyTicket; }
  ...
  function init(){ bindDom(); load(); setupAppleMotion(); buttonRipples(); let progress=0; const loadTimer=setInterval(()=>{ progress+=18+Math.random()*16; dom.loaderBar.style.width=Math.min(100,progress)+'%'; if(progress>=100){ clearInterval(loadTimer); dom.startBtn.classList.remove('hidden'); } },160);
    ...
    setScreen(state.screen||'home'); render(); requestAnimationFrame(loop); if('serviceWorker' in navigator){ navigator.serviceWorker.register('./sw.js?v=11.0.0-boss-ticket-shop-only').catch(()=>{}); }
  }
  init();
})();
